...
intros c1 [H1 [H2 | [H2 | [H2 | [H2 | [H2 | H2]]]]]]; rewrite H1;
  rewrite H2; clear H1 H2;
  intros c2 [H1 [H2 | [H2 | [H2 | [H2 | [H2 | H2]]]]]]; rewrite H1;
    rewrite H2; clear H1 H2; case s; unfold tc6i, tc6a;
    try (intros; apply int63_0; auto with zarith);
    ((intros i1 i2 i3 i4 i5 i6 H1 [HH | HH]) || 
     (intros _ [HH | HH]));
...
Lemma tc5ai s l p : validtc5 l -> valid63 p ->
  tc5i l s p = false -> tc5i l (tc5a l s p) p = true.
Proof.
intros H Hp.
assert (Hp1: (Zpos p <= 63)%Z); auto.
case (validtc5_inv _ H); clear H.
intros p1 [l1 [H1 [[H2 | [H2 | [H2 | [H2 | H2]]]] H3]]]; rewrite H1;
  rewrite H2; clear H1 H2; case s; unfold tc5i, tc5a; auto;
  intros; apply tc6ai; auto.
Qed.

Lemma tc5aif s l1 l2 p1 p2 : validtc5 l1 -> validtc5 l2  -> 
  valid63 p1 -> valid63 p2 ->
  tc5i l1 s p1 = false -> (l1 <> l2 \/ p1 <> p2) -> tc5i l2 (tc5a l1 s p1) p2 = tc5i l2 s p2.
Proof.
intros Hl1 Hl2 Hp1 Hp2; case (validtc5_inv _ Hl1); clear Hl1;
  case (validtc5_inv _ Hl2); clear Hl2.
assert (Hpp1: (Zpos p1 <= 63)%Z); auto.
assert (Hpp2: (Zpos p2 <= 63)%Z); auto.
intros c1 [l3 [H1 [[H2 | [H2 | [H2 | [H2 | H2]]]] Hp]]]; rewrite H1;
  rewrite H2; clear H1 H2; 
intros q1 [l4 [H1 [[H2 | [H2 | [H2 | [H2 | H2]]]] Hq]]]; rewrite H1;
  rewrite H2; clear H1 H2; case s; unfold tc5i, tc5a; auto;
  intros; rewrite tc6aif; auto;
  try (match goal with H : _ \/ _ |- _  => case H end;
       auto; intros HH; left; intros HH1; case HH; rewrite HH1; auto).
Qed.

Lemma tc4ai s l p : validtc4 l -> valid63 p ->
...
intros q1 [l4 [H1 [[H2 | [H2 | [H2 | H2]]] Hq]]]; rewrite H1;
  rewrite H2; clear H1 H2; case s; unfold tc4i, tc4a; auto;
  intros; rewrite tc5aif; auto;
  try (match goal with H : _ \/ _ |- _  => case H end;
       auto; intros HH; left; intros HH1; case HH; rewrite HH1; auto).
Qed.

Lemma to1ai s l l1 p : validto1 l -> validtc4 l1 -> valid63 p ->
  to1i l s l1 p = false -> to1i l (to1a l s l1 p) l1 p = true.
Proof.
intros Hl Hl1 Hp; generalize Hl; case l; clear l Hl.
intros HH; discriminate.
intros o1 l _; case o1; case s; simpl; intros; apply tc4ai; auto.
Qed.

Lemma to1aif s l1 l2 l3 l4 p1 p2 : validto1 l1 -> validto1 l2  ->
  validtc4 l3 -> valid63 p1 -> validtc4 l4 -> valid63 p2 ->
...
Lemma to2ai s l l1 p : validto2 l -> validtc4 l1 -> valid63 p ->
...
Lemma to3ai s l l1 p : validto3 l -> validtc4 l1 -> valid63 p ->
...
Lemma to4ai s l l1 p : validto4 l -> validtc4 l1 -> valid63 p ->
...
Lemma to5ai s l l1 p : validto5 l -> validtc4 l1 -> valid63 p ->
...
Lemma tc2ai s l l1 p : validtc2 l -> validto5 l1 -> valid63 p ->
...
intros p1 [l2  [H1 [[H2 | H2] H3]]]; rewrite H1;
  ...
  intros; apply to5ai; auto.
Qed.

Lemma tc2aif s l1 l2 l3 l4 p1 p2 : validtc2 l1 -> validtc2 l2  ->
  validto5 l3 -> valid63 p1 -> validto5 l4 -> valid63 p2 ->
...
Definition naive_int63it l1 l2 a c := naive_int63it_aux 62 1 c l1 l2 a.
...
Proof.
revert a l1 l2 c.
assert (F:  forall m p c c1 l1 l2 a,
  (Z_of_nat m + Zpos p = 63)%Z -> 
  to_Z c1 = to_Z (shiftr63 c (p2t63 p)) ->
     int63it_aux _ next (S m) p c1 l1 l2  a = naive_int63it_aux m p c l1 l2 a).
- intros m; elim m; clear m; auto.
  + intros p c c1 l1 l2 a H1 H2.
    ...
    * apply f_equal2 with (f := Z.mul); auto.
      rewrite 2!phi_shiftr63; rewrite ?phi1; auto with zarith.
      now apply f_equal2 with (f := Z.div); auto.
    * rewrite phi_shiftr63; rewrite phi1; auto with zarith.
      change (2^63)%Z with (2^62 * 2 ^1)%Z.
      apply Zmult_lt_compat_r; auto with zarith.
      apply Z.div_lt_upper_bound; auto with zarith.
      change (2 ^ 1 * 2 ^ 62)%Z with (2 ^ 63)%Z.
      now rewrite <- H2; case (phi_toZ c1); auto.
    * change (2^63)%Z with (2^62 * 2 ^1)%Z.
      apply Zmult_lt_compat_r; auto with zarith.
      rewrite phi_shiftr63; rewrite phi1; auto with zarith.
      apply Z.div_lt_upper_bound; auto with zarith.
      change (2 ^ 1 * 2 ^ 62)%Z with (2 ^ 63)%Z.
      now case (phi_toZ c1); auto.
  + intros m Hrec p c c1 l1 l2 a H1 H2.
    ...
    assert (HH : to_Z (shiftr63 c1 1) =
                 to_Z (shiftr63 c (p2t63 (Pos.succ p)))).
    * rewrite phi_shiftr63, H2, !phi_shiftr63; 
        rewrite ?phi_p2t63;
        rewrite ?Zpos_succ_morphism; 
        rewrite ?phi1; auto with zarith.
      rewrite Zdiv_Zdiv; auto with zarith.
      rewrite <- Zpower_exp; auto with zarith.
    ...
    * match goal with |- context[compare ?X  ?Y] =>
      ...
      repeat rewrite phi_shiftl63; rewrite phi1; auto with zarith.
      -- apply f_equal2 with (f := Z.mul); auto.
         rewrite HH, !phi_shiftr63.
         ++ rewrite Zdiv_Zdiv; auto with zarith.
            rewrite <- Zpower_exp; auto with zarith.
            ** apply f_equal2 with (f := Z.div); auto.
               rewrite !phi_p2t63; rewrite ?phi1, ?Zpos_succ_morphism; auto with zarith.
               apply f_equal2 with (f := Zpower); auto with zarith.
            ** now generalize (phi_nonneg (p2t63 p)); auto with zarith.
            ** now rewrite phi1.
          ++ now rewrite phi_p2t63; auto with zarith.
          ++ now rewrite phi1.
          ++ rewrite phi_p2t63, Zpos_succ_morphism.
               now rewrite <- H1; auto with zarith.
        ...
        -- change (2^63)%Z with (2^62 * 2 ^1)%Z.
           ...
           apply Z.div_lt_upper_bound; auto with zarith.
           change (2 ^ 1 * 2 ^ 62)%Z with (2 ^ 63)%Z.
           now case (phi_toZ (shiftr63 c (p2t63 p))); auto.
        -- change (2^63)%Z with (2^62 * 2 ^1)%Z.
           apply Zmult_lt_compat_r; auto with zarith.
           rewrite phi_shiftr63; rewrite phi1; auto with zarith.
           apply Z.div_lt_upper_bound; auto with zarith.
...
Theorem make_list63i l1 l2 c k ll :
  In k (int63it _ make_list l1 l2 ll c) ->
   In k ll \/
   exists p, (Zpos p <= 63)%Z /\ k = (l1,l2,p) /\ int63i c p = true.
Proof.
...
In k ll \/
exists p1 : positive, (Zpos p <= Zpos p1 <= 63)%Z /\ 
...
- intros m; elim m; clear m.
  ...
  + intros m Hrec p l1 l2 c k ll H.
    rewrite naive_auxS.
    assert (F1: (Z_of_nat  m + (Zpos (Pos.succ p)) = 63)%Z).
      rewrite <- H.
...
assert(F:
  ...
  (Z_of_nat m + Zpos p = 63)%Z -> 
    (Zpos p <= Zpos p1 <= 63)%Z -> int63i c p1 = true ->
  In (l1,l2,p1)
   (naive_int63it_aux _
...
generalize (1%positive); elim 62%nat.
...
Theorem make_listtc6i l1 l2 c k ll :
   In k (tc6it _ make_list l1 l2 ll c) -> 
   In k ll \/(exists l3, exists p, k = (l3 ++ l1,l2,p) 
               /\ validtc6 (rev l3) /\ valid63 p 
                  /\ tc6i (rev l3) c p = true).
Proof.
revert ll; unfold tc6it; case c.
  now intros ll HH; left; assumption.
intros cc1 cc2 cc3 cc4 cc5 cc6 ll.
set (ll1 := (int63it _ make_list (C6 :: l1) l2 ll cc6)).
set (ll2 := (int63it _ make_list (C5 :: l1) l2 ll1 cc5)).
set (ll3 := (int63it _ make_list (C4 :: l1) l2 ll2 cc4)).
set (ll4 := (int63it _ make_list (C3 :: l1) l2 ll3 cc3)).
set (ll5 := (int63it _ make_list (C2 :: l1) l2 ll4 cc2)).
intros H.
...
- case (make_list63i _ _ _ _ _ H); clear H; unfold ll4; intros H.
         ...
         ++ case (make_list63i _ _ _ _ _ H); clear H; intros H.
            ...
            ** case H; intros p (Hp1, (Hp2, Hp3)).
               ...
               exists (C6 :: nil); exists p; split.
                 now reflexivity.
               split.
                 now unfold validtc6, valid63, rev, app; auto.
                 now split; auto with zarith.
         ++ case H; intros p (Hp1, (Hp2, Hp3)).
            right; rewrite Hp2.
  ...
  + case H; intros p (Hp1, (Hp2, Hp3)).
    ...
    exists (C2 :: nil); exists p; split.
      now reflexivity.
    split.
      unfold validtc6, valid63, rev, app; simpl; auto with arith.
    split; auto.
- case H; intros p (Hp1, (Hp2, Hp3)).
  right; rewrite Hp2.
  exists (C1 :: nil); exists p; split.
    now reflexivity.
  split.
    now unfold validtc6, valid63, rev, app; auto with arith.
  split; auto.
Qed.

Theorem make_listtc6irA c l1 l2 ll1 ll2 :
   tc6it _ make_list l1 l2 (ll1 ++ ll2) c =
   ll1 ++ tc6it _ make_list l1 l2 ll2 c.
Proof.
case c; unfold tc6it; auto with datatypes.
intros cc1 cc2 cc3 cc4 cc5 cc6; repeat rewrite make_list63irA.
reflexivity.
Qed.

Theorem make_listtc6irI k c l1 l2 ll :
   In k ll -> In k (tc6it _ make_list l1 l2 ll c).
Proof.
intro H; replace ll with (ll ++ nil);
 try rewrite make_listtc6irA; auto with datatypes.
Qed.

Theorem make_listtc6ir l p c l1 l2 ll :
   validtc6 (rev l) -> valid63 p -> tc6i (rev l) c p = true -> 
   In (l ++ l1,l2,p) (tc6it _ make_list l1 l2 ll c).
Proof.
(* Hack to freeze the simplification of tc7it *)
assert (H : exists f, f = tc6it).
  now exists tc6it; auto.
case H; intros f Hf; rewrite <- Hf.
case c.
  now unfold tc6i; intros _ _ HH; discriminate.
intros cc1 cc2 cc3 cc4 cc5 cc6 HH Hp.
case (validtc6_inv _ HH); clear HH.
intros p1 (HH, HH1).
rewrite <- (rev_involutive l).
rewrite HH; simpl rev; clear HH.
case HH1; [intros HH2 | intros [HH2 | [HH2 | [HH2 | [HH2 | HH2]]]]];
...
Theorem make_listtc6fold l1 l2 a c :
  tc6it _ next l1 l2 a c =
  fold_left nextp (tc6it _ make_list l1 l2 nil c) a.
Proof.
case c.
  now reflexivity.
...
Theorem make_listtc5i l1 l2 c k ll :
   In k (tc5it _ make_list l1 l2 ll c) -> 
   In k ll \/(exists l3, exists p, k = (l3 ++ l1,l2,p) 
               /\ validtc5 (rev l3) /\ valid63 p 
                  /\ tc5i (rev l3) c p = true).
Proof.
revert ll; unfold tc5it; case c.
  now intros ll HH; left; assumption.
intros cc1 cc2 cc3 cc4 cc5 ll.
set (ll1 := (tc6it _ make_list (C5 :: l1) l2 ll cc5)).
set (ll2 := (tc6it _ make_list (C4 :: l1) l2 ll1 cc4)).
set (ll3 := (tc6it _ make_list (C3 :: l1) l2 ll2 cc3)).
set (ll4 := (tc6it _ make_list (C2 :: l1) l2 ll3 cc2)).
intros H.
case (make_listtc6i _ _ _ _ _ H); clear H; unfold ll4; intros H.
- case (make_listtc6i _ _ _ _ _ H); clear H; unfold ll3; intros H.
  + case (make_listtc6i _ _ _ _ _ H); clear H; unfold ll2; intros H.
    * case (make_listtc6i _ _ _ _ _ H); clear H; unfold ll1; intros H.
      -- case (make_listtc6i _ _ _ _ _ H); clear H; intros H.
           now left; assumption.
...
Theorem make_listtc5irA c l1 l2 ll1 ll2 :
   tc5it _ make_list l1 l2 (ll1++ll2) c =
   ll1 ++ tc5it _ make_list l1 l2 ll2 c.
Proof.
case c; unfold tc5it; auto with datatypes.
intros cc1 cc2 cc3 cc4 cc5; repeat rewrite make_listtc6irA; auto.
Qed.

Theorem make_listtc5irI k c l1 l2 ll :
   In k ll -> In k (tc5it _ make_list l1 l2 ll c).
Proof.
intro H; replace ll with (ll ++ nil);
 try rewrite make_listtc5irA; auto with datatypes.
Qed.

Theorem make_listtc5ir l p c l1 l2 ll :
   validtc5 (rev l) ->  valid63 p -> tc5i (rev l) c p = true -> 
   In (l ++ l1, l2, p) (tc5it _ make_list l1 l2 ll c).
Proof.
case c.
  now unfold tc5i; intros _ _ HH; discriminate.
intros cc1 cc2 cc3 cc4 cc5 HH Hp.
case (validtc5_inv _ HH); clear HH.
...
- do 2 (apply  make_listtc6irI); 
        apply make_listtc6ir; auto; rewrite rev_involutive; auto.
- do 3 (apply  make_listtc6irI); 
        apply make_listtc6ir; auto; rewrite rev_involutive; auto.
- do 4 (apply  make_listtc6irI); 
        apply make_listtc6ir; auto; rewrite rev_involutive; auto.
Qed.

Theorem make_listtc5fold l1 l2 a c :
  tc5it _ next l1 l2 a c =
  fold_left nextp (tc5it _ make_list l1 l2 nil c) a.
Proof.
case c.
  now reflexivity.
...
Theorem make_listtc4i l1 l2 c k ll :
   ...
   In k ll \/(exists l3, exists p, k = (l3 ++ l1,l2,p) 
               /\ validtc4 (rev l3) /\ valid63 p 
                  /\ tc4i (rev l3) c p = true).
Proof.
revert ll; unfold tc4it; case c.
  now intros ll HH; left; assumption.
intros cc1 cc2 cc3 cc4 ll.
set (ll1 := (tc5it _ make_list (C4 :: l1) l2 ll cc4)).
set (ll2 := (tc5it _ make_list (C3 :: l1) l2 ll1 cc3)).
set (ll3 := (tc5it _ make_list (C2 :: l1) l2 ll2 cc2)).
set (ll4 := (tc5it _ make_list (C1 :: l1) l2 ll3 cc1)).
intros H.
case (make_listtc5i _ _ _ _ _ H); clear H; unfold ll4; intros H.
- case (make_listtc5i _ _ _ _ _ H); clear H; unfold ll3; intros H.
  + case (make_listtc5i _ _ _ _ _ H); clear H; unfold ll2; intros H.
    * case (make_listtc5i _ _ _ _ _ H); clear H; unfold ll1; intros H.
        now left; assumption.
      case H; intros l3 (p, (Hp1, (Hp2, (Hp3, Hp4)))).
      right; rewrite Hp1.
      exists (l3 ++ (C4 :: nil)); exists p; split.
...
intros cc1 cc2 cc3 cc4; repeat rewrite make_listtc5irA; auto.
Qed.
...
Theorem make_listtc4ir l p c l1 l2 ll :
   validtc4 (rev l) ->  valid63 p -> tc4i (rev l) c p = true -> 
...
Theorem make_listto1i l1 l2 c k ll :
   ...
   In k ll \/(exists l3, exists l4, exists p, k = (l3++l1,l4++l2,p) 
               /\ validtc4 (rev l3) /\ validto1 (rev l4) /\ valid63 p 
                  /\ to1i (rev l4) c (rev l3) p = true).
Proof.
revert ll; unfold to1it; case c.
  now intros ll HH; left; assumption.
intros cc1 cc2 cc3 ll.
set (ll1 := (tc4it _ make_list l1 (O3 :: l2) ll cc3)).
set (ll2 := (tc4it _ make_list l1 (O2 :: l2) ll1 cc2)).
set (ll3 := (tc4it _ make_list l1 (O1 :: l2) ll2 cc1)).
intros H.
...
Theorem make_listto1ir l1 l2 p c l3 l4 ll :
   validtc4 (rev l1) -> validto1 (rev l2) ->  valid63 p -> 
...
- apply  make_listtc4irI; apply make_listtc4ir; auto.
- do 2 (apply  make_listtc4irI); apply make_listtc4ir; auto.
Qed.
...
Theorem make_listto2i l1 l2 c k ll :
   ...
   In k ll \/(exists l3, exists l4, exists p, k = (l3++l1,l4++l2,p) 
               /\ validtc4 (rev l3) /\ validto2 (rev l4) /\ valid63 p 
...
Theorem make_listto2ir l1 l2 p c l3 l4 ll :
   validtc4 (rev l1) -> validto2 (rev l2) ->  valid63 p -> 
...
Theorem make_listto3i l1 l2 c k ll :
   ...
   In k ll \/(exists l3, exists l4, exists p, k = (l3++l1,l4++l2,p) 
               /\ validtc4 (rev l3) /\ validto3 (rev l4) /\ valid63 p 
...
Theorem make_listto3ir l1 l2 p c l3 l4 ll :
   validtc4 (rev l1) -> validto3 (rev l2) ->  valid63 p -> 
...
Theorem make_listto4i l1 l2 c k ll :
   ...
   In k ll \/(exists l3, exists l4, exists p, k = (l3++l1,l4++l2,p) 
               /\ validtc4 (rev l3) /\ validto4 (rev l4) /\ valid63 p 
...
Theorem make_listto4ir l1 l2 p c l3 l4 ll :
   validtc4 (rev l1) -> validto4 (rev l2) ->  valid63 p -> 
...
Theorem make_listto5i l1 l2 c k ll :
   ...
   In k ll \/(exists l3, exists l4, exists p, k = (l3++l1,l4++l2,p) 
               /\ validtc4 (rev l3) /\ validto5 (rev l4) /\ valid63 p 
...
Theorem make_listto5ir l1 l2 p c l3 l4 ll :
   validtc4 (rev l1) -> validto5 (rev l2) ->  valid63 p -> 
...
Theorem make_listtc2i c k ll :
   ...
   In k ll \/(exists l3, exists l4, exists p, k = (l3,l4,p) 
               /\ validtc2 (rev l3) /\ validto5 (rev l4) /\ valid63 p 
                  /\ tc2i (rev l3) (rev l4) p c = true).
Proof.
unfold tc2it; case c.
  now intros HH; left; assumption.
intros cc1 cc2.
set (ll1 := (to5it _ make_list (C2 :: nil) nil ll cc2)).
set (ll2 := (to5it _ make_list (C1 :: nil) nil ll1 cc1)).
intros H.
...
intros cc1 cc2; repeat rewrite make_listto5irA; auto.
Qed.
...
Theorem make_listtc2ir l1 l2 p c ll :
   validtc2 (rev l1) -> validto5 (rev l2) -> valid63 p ->
...
Lemma check63_all c : int63all c = true -> 
  forall s, (Zpos s <= 63)%Z -> int63i c s = true.
Proof.
unfold int63all.
generalize (phi_compare63 c 9223372036854775807); case compare;
  try (intros; discriminate).
intros H1 _ s Hs.
assert (F0: (Zpos s <= 63)%Z); auto with zarith.
generalize (spec_int63i c s F0); rewrite H1.
case int63i; auto.
change (to_Z 9223372036854775807)%Z with (9223372036854775807)%Z.
generalize s Hs; clear s Hs F0.
do 6 (intros s; case s; clear s;
      [idtac|idtac| match goal with |- context[?X = 0%Z] =>
                     change X with 1%Z end;
                     intros; discriminate]);
try (now intros p HH; contradict HH; auto with zarith).
Qed.

Lemma checktc6_all : forall c, tc6all c = true -> 
  forall l p, validtc6 l -> valid63 p -> tc6i l c p = true.
Proof.
intros c; case c; [intros HH; discriminate | idtac].
intros p1 p2 p3 p4 p5 p6 Hc l p Hl; simpl in Hc; generalize Hc;
  clear Hc; case (validtc6_inv _ Hl).
intros c1 (Hc1, [H2 | [H2 |  [H2 | [H2 | [H2 | H2]]]]]); 
  rewrite Hc1; rewrite H2; unfold tc6i;
  intros Ha Hp;
...
Lemma checktc5_all c l p : tc5all c = true -> 
  validtc5 l -> valid63 p -> tc5i l c p = true.
Proof.
case c; [intros HH; discriminate | idtac].
intros p1 p2 p3 p4 p5 Hc Hl; simpl in Hc; generalize Hc;
  clear Hc; case (validtc5_inv _ Hl).
intros c1 (l1, (Hl1, [[H2 | [H2 | [H2 | [H2 | H2]]]] Hl2]));
  rewrite Hl1; rewrite H2; unfold tc5i;
  intros Hp; apply checktc6_all; auto;
  generalize Hp; do 5 (case tc6all; auto).
Qed.

Lemma checktc4_all c l p : tc4all c = true -> 
  validtc4 l -> valid63 p -> tc4i l c p = true.
Proof.
case c; [intros HH; discriminate | idtac].
...
intros c1 (l1, (Hl1, [[H2 | [H2 | [H2 | H2]]] Hl2]));
  ...
  intros Hp; apply checktc5_all; auto;
  generalize Hp; do 4 (case tc5all; auto).
Qed.

Lemma checkto1_all c l1 l2 p : to1all c = true -> 
  validtc4 l1 -> validto1 l2 -> valid63 p ->
  to1i l2 c l1 p = true.
Proof.
case c; [intros HH; discriminate | idtac].
...
case o1; unfold to1i; intros; apply checktc4_all; auto;
  generalize Hc; do 3 (case tc4all; auto).
Qed.

Lemma checkto2_all c l1 l2 p : to2all c = true -> 
  validtc4 l1 -> validto2 l2 -> valid63 p ->
  to2i l2 c l1 p = true.
Proof.
case c; [intros HH; discriminate | idtac].
intros p1 p2 p3 Hc Hl; simpl in Hc; generalize Hc;
...
Lemma checkto3_all c l1 l2 p : to3all c = true -> 
  validtc4 l1 -> validto3 l2 -> valid63 p ->
  to3i l2 c l1 p = true.
Proof.
case c; [intros HH; discriminate | idtac].
intros p1 p2 p3 Hc Hl; simpl in Hc; generalize Hc;
...
Lemma checkto4_all c l1 l2 p : to4all c = true -> 
  validtc4 l1 -> validto4 l2 -> valid63 p ->
  to4i l2 c l1 p = true.
Proof.
case c; [intros HH; discriminate | idtac].
intros p1 p2 p3 Hc Hl; simpl in Hc; generalize Hc;
...
Lemma checkto5_all c l1 l2 p : to5all c = true -> 
  validtc4 l1 -> validto5 l2 -> valid63 p ->
  to5i l2 c l1 p = true.
Proof.
case c; [intros HH; discriminate | idtac].
intros p1 p2 p3 Hc Hl; simpl in Hc; generalize Hc;
...
Lemma checktc2_all c l1 l2 p : tc2all c = true -> 
     validtc2 l1 -> validto5 l2 -> valid63 p ->
     tc2i l1 l2 p c = true.
Proof.
case c; [intros HH; discriminate | idtac].
...
intros c1 [l3 [Hl1 [[H2 | H2] Hl2]]];
  rewrite Hl1; rewrite H2; unfold tc4i;
  intros Hp; apply checkto5_all; auto;
  generalize Hp; do 3 (case to5all; auto).
Qed.
...
Lemma iter_final n :
  let (s1,s2) := iter n in
   match s2 with 
    OTC2 => forall s, reachable s -> nlreachable (Nat.pred n) s
   | _  => True
   end.
Proof.
...
Lemma tc2_solve_length n s s1 s2 :
  (s1, s2) = fst (iter2 n) ->
  valid_state s -> 
  (length (tc2_solve n s s1 s2) <= n)%nat.
Proof.
intros H1 H.
