Require Import Int31 Cyclic31 List BasicRubik.

(** Cubes positions. There are 7*6*5*4*3*2 positions.                        **)
(** We code them as 7 * 4 * 3 * 2 * (6 * 5 = 30)                             **) 

Inductive tc7 := OTC7 | TC7 (_ _ _ _ _ _ _: int31).
...
Inductive tc2 := OTC2: tc2 | TC2 (_ _ : to6).

(** Some 31 bits operations                                                  **)

Open Scope int31_scope.

Definition p2t31 x := snd (positive_to_int31 x) - 1.

Definition shiftr31 x n := addmuldiv31 (31 - n) 0 x.
Definition shiftl31 x n := addmuldiv31 n x 0.

Eval  native_compute in shiftr31 24 2.

(** Check if a given position is in a state                                  **)

Definition int31i x n := 
  let u := p2t31 n in
  let v := shiftr31 x u in
  let w := shiftl31 (shiftr31 v 1) 1 in
  match compare31  v w with Eq => false | _ => true end.

Definition tc7i l c n:= 
  match c, l  with 
  | TC7 m _ _ _ _ _ _, C1 :: _ => int31i m n
  | TC7 _ m _ _ _ _ _, C2 :: _ => int31i m n
  | TC7 _ _ m _ _ _ _, C3 :: _ => int31i m n
  | TC7 _ _ _ m _ _ _, C4 :: _ => int31i m n
  | TC7 _ _ _ _ m _ _, C5 :: _ => int31i m n
  | TC7 _ _ _ _ _ m _, C6 :: _ => int31i m n
  | TC7 _ _ _ _ _ _ m, C7 :: _ => int31i m n
  | _, _ => false
  end.
...
Definition tc2i l l1 n c:= 
  match c, l  with 
  | TC2 m _, C1 :: l2 => to6i l1 m l2 n
  | TC2 _ m, C2 :: l2 => to6i l1 m l2 n
  | _, _ => false
  end.

(** Add a position in a state                                                **)

Definition int31a x n := 
  add31 x (shiftl31 1 (p2t31 n)).

Definition tc7a l c n := 
  match l, c  with 
  | C1 :: _, TC7 m m0 m1 m2 m3 m4 m5 => TC7 (int31a m n) m0 m1 m2 m3 m4 m5
  | C2 :: _, TC7 m0 m m1 m2 m3 m4 m5 => TC7 m0 (int31a m n) m1 m2 m3 m4 m5
  | C3 :: _, TC7 m0 m1 m m2 m3 m4 m5 => TC7 m0 m1 (int31a m n) m2 m3 m4 m5
  | C4 :: _, TC7 m0 m1 m2 m m3 m4 m5 => TC7 m0 m1 m2 (int31a m n) m3 m4 m5
  | C5 :: _, TC7 m0 m1 m2 m3 m m4 m5 => TC7 m0 m1 m2 m3 (int31a m n) m4 m5
  | C6 :: _, TC7 m0 m1 m2 m3 m4 m m5 => TC7 m0 m1 m2 m3 m4 (int31a m n) m5
  | C7 :: _, TC7 m0 m1 m2 m3 m4 m5 m => TC7 m0 m1 m2 m3 m4 m5 (int31a m n)
  | C1 :: _, OTC7 => TC7 (int31a 0 n) 0 0 0 0 0 0
  | C2 :: _, OTC7 => TC7 0 (int31a 0 n) 0 0 0 0 0
  | C3 :: _, OTC7 => TC7 0 0 (int31a 0 n) 0 0 0 0
  | C4 :: _, OTC7 => TC7 0 0 0 (int31a 0 n) 0 0 0 
  | C5 :: _, OTC7 => TC7 0 0 0 0 (int31a 0 n) 0 0
  | C6 :: _, OTC7 => TC7 0 0 0 0 0 (int31a 0 n) 0
  | C7 :: _, OTC7 => TC7 0 0 0 0 0 0 (int31a 0 n)
  | _, _ => c
  end.
...
Definition tc2a l l1 n c := 
  match l, c  with 
  | C1 :: l2, TC2 m m0 => TC2 (to6a l1 m l2 n) m0
  | C2 :: l2, TC2 m0 m => TC2 m0 (to6a l1 m l2 n)
  | C1 :: l2, OTC2 => TC2 (to6a l1 OTO6 l2 n) OTO6
  | C2 :: l2, OTC2 => TC2 OTO6 (to6a l1 OTO6 l2 n)
  | _, _ => c
  end.
...
Fixpoint int31it_aux (m : nat) (p : positive) 
                     (c1 tc2 : int31) (lc : list cube) (lo : list orientation)
                     (a : A) {struct m} :=
 match m with 
 | O => a 
 | S m1 => 
    let a1 := match compare31 c1 (shiftl31 tc2 1) with 
              | Eq => a 
              | _  => next a lc lo p 
              end in
    int31it_aux m1 (Pos.succ p) tc2 (shiftr31 tc2 1) lc lo a1
 end.

Definition int31it a lc lo c := int31it_aux 30 1%positive c (shiftr31 c 1) a lc lo.
     ...
     (int31it (C2 :: lc) lo  
       (int31it (C3 :: lc) lo 
         (int31it (C4 :: lc) lo
           (int31it (C5 :: lc) lo
             (int31it (C6 :: lc) lo
               (int31it (C7 :: lc) lo a m6) m5) m4) m3) m2) m1) m0
  | _ => a
  end.
...
Definition to6it lc lo a c := 
  match c  with 
  | TO6 m0 m1 m2 => to5it lc (O1 :: lo) 
                    (to5it lc (O2 :: lo)  
                      (to5it lc (O3 :: lo) a m2) m1) m0
  | _ => a
  end.
...
Definition tc2it a c := 
  match c  with 
  | TC2 m0 m1 => to6it (C1  ::  nil) nil (to6it (C2  ::  nil) nil a m1) m0
  | _ => a
  end.

End Iter.

Eval  native_compute in int31it _ (fun x l1 l2 y => y :: x) nil nil nil 2345.

 
(** Check that all states have been checked                                  **)

Definition int31all c:=
  match compare31 c 1073741823 with Eq => true | _ => false end.

Definition tc7all c := 
  match c  with 
  | TC7 m0 m1 m2 m3 m4 m5 m6 => 
   (int31all m0 && int31all m1 && int31all m2 && int31all m3 && 
    int31all m4 && int31all m5 && int31all m6)%bool
  | _ => false
  end.
...
Definition encode_state s := 
  ...
  | State i7 i6 i5 i4 i3 i2 i1 o1 o2 o3 o4 o5 o6 o7 =>
    ...
    | p7 :: p6 :: p5 :: p4 :: p3 :: p2 :: l => 
      (p2 :: p3 :: p4 :: p7 :: nil,
       o6 :: o5 :: o4 :: o3 :: o2 :: o1 :: nil, cube_encode p5 p6)
...
Definition decode_state lc lo n := 
  match lc, lo with
  | p7 :: p4 :: p3 :: p2 :: nil, o1 :: o2 :: o3 :: o4 :: o5 :: o6 :: nil => 
       let (p5, p6) := cube_decode n in 
...
(*****************************************************************************)
(*                                                                           *)
(*           Some properties of int31                                        *)
(*                                                                           *)
(*****************************************************************************)

Lemma phi_addmuldiv31 (x y p : int31):
     (phi p <= 31)%Z ->
     (phi (addmuldiv31 p x y) =
       ((phi x * 2 ^ phi p +  phi y / 2 ^ (31 - phi p)) mod 2^31))%Z.
Proof. apply spec_add_mul_div. Qed.

Definition phi0: phi 0 = 0%Z := (refl_equal 0%Z).
Definition phi1: phi 1 = 1%Z := (refl_equal 1%Z).
Definition phi31: phi 31 = 31%Z := (refl_equal 31%Z).

Definition phi_to_Z (x : int31) : (0 <= phi x < 2 ^ 31)%Z := (phi_bounded x).

Definition phi_add (x y : int31) :  
     (phi (x + y) = ((phi x + phi y) mod 2 ^ 31))%Z := (spec_add x y).

Definition phi_sub (x y : int31) : 
     (phi (x - y) = ((phi x - phi y) mod 2 ^ 31))%Z := (spec_sub x y).

Lemma phi_shiftr31 p n :
  (phi n <= 31)%Z -> phi (shiftr31 p n) = (phi p / 2 ^ (phi n))%Z.
Proof.
intro H; unfold shiftr31.
case (phi_to_Z n); intros Hn1 Hn2.
case (phi_to_Z p); intros Hp1 Hp2.
rewrite phi_addmuldiv31; auto with zarith.
2: rewrite phi_sub; rewrite Zmod_small; rewrite phi31; auto with zarith.
2: split; auto with zarith.
2: apply Z.le_lt_trans with 31%Z; auto with zarith; red; auto.
rewrite phi0; rewrite Zmult_0_l; rewrite Zplus_0_l.
rewrite phi_sub; rewrite phi31.
rewrite (Zmod_small (31 - phi n)%Z).
2: split; auto with zarith.
2: apply Z.le_lt_trans with 31%Z; auto with zarith; red; auto.
replace (31 - (31 - phi n))%Z with (phi n); auto with zarith.
...
apply Z.le_lt_trans with (phi p/ 2 ^ 0)%Z; auto with zarith.
...
Lemma phi_shiftl31 p n :
  (phi n <= 31)%Z -> (phi p * 2 ^ (phi n) < 2 ^31)%Z -> 
  (phi (shiftl31 p n) = phi p * (2 ^ (phi n)))%Z.
Proof.
intros H H1; unfold shiftl31.
case (phi_to_Z n); intros Hn1 Hn2.
case (phi_to_Z p); intros Hp1 Hp2.
rewrite phi_addmuldiv31; auto with zarith.
...
Lemma phi_p2t31 p : 
  (Zpos p <= 31 -> phi (p2t31 p) = Zpos p - 1)%Z.
Proof.
intros Hp; unfold p2t31.
rewrite phi_sub; rewrite phi1.
assert (Z.pos p = phi (snd (positive_to_int31 p))).
  generalize Hp.
  rewrite (positive_to_int31_spec p).
  case (fst (positive_to_int31 p)); auto.
  intros p1 H; contradict H.
  rewrite N2Z.inj_pos.
  apply Zlt_not_le.
  replace 31%Z with (1 * 31 + 0)%Z; auto with zarith.
...
split; auto with zarith.
apply Z.lt_trans with (31)%Z; auto with zarith.
Qed.

Local Definition phi_compare31 (x y: int31) :
     match x ?= y with 
      Lt => (phi x < phi y)%Z
    | Gt => (phi y <  phi x)%Z
    | Eq => (phi x = phi y)%Z
...
Definition valid31 p := (p < 31)%positive.
...
Lemma validtc2_inv l : validtc2 l ->
 exists p1, exists l1, l = p1 :: l1 /\ (p1 = C1 \/ p1 = C2) /\ (validtc3 l1).
Proof.
case l; clear l; try (intros HH; case HH; fail).
intros p1 l1 [Hp1 Hl1]; exists p1; exists l1; split; auto.
generalize Hp1; case p1; simpl; auto;
...
(* int31i is correct *)
Lemma spec_int31i s p : (Zpos p <= 31)%Z ->
   (phi s/2^(Zpos p - 1) mod 2 = if int31i s p then 1 else 0)%Z.
Proof.
intro H; unfold int31i.
case (phi_to_Z s); intros Hs1 Hs2.
assert (F1: (phi 1 <= 31)%Z).
  rewrite phi1; intros HH; discriminate.
assert (F2: (phi (shiftr31 (shiftr31 s (p2t31 p)) 1) = phi s / 2 ^ (Zpos p))%Z).
  repeat (rewrite phi_shiftr31; auto); rewrite phi_p2t31; auto with zarith.
...
assert (F3: (phi (shiftr31 (shiftr31 s (p2t31 p)) 1) * 2 ^ phi 1 < 2 ^ 31)%Z).
  rewrite phi1; rewrite F2; rewrite Zpow_facts.Zpower_1_r.
  change (2^31)%Z with (2^30 * 2)%Z.
  apply Zmult_lt_compat_r; auto with zarith.
  apply Zdiv_lt_upper_bound; auto with zarith.
  apply Z.lt_le_trans with (1 := Hs2).
  change (2 ^ 31)%Z with (2 ^ 30 * 2 ^ 1)%Z.
  apply Zmult_le_compat_l; auto with zarith.
  apply Zpow_facts.Zpower_le_monotone; auto with zarith.
  split; auto with zarith.
  assert (0 < Zpos p)%Z; auto with zarith; red; auto.
assert (F4: (phi (p2t31 p) <= 31)%Z).
  rewrite phi_p2t31; auto with zarith.
...
match goal with |- context[?X ?= ?Y] => 
  generalize (phi_compare31 X Y); case compare31
end; rewrite phi_shiftl31; auto; rewrite F2; rewrite phi_shiftr31; auto;
  rewrite phi1; rewrite Zpow_facts.Zpower_1_r; rewrite phi_p2t31; auto;
  set (u := (phi s/ (2 ^ (Zpos p - 1)))%Z); pattern u at 1; 
...
Lemma int31ai s p : (Zpos p <= 31)%Z ->
  int31i s p = false -> int31i (int31a s p) p = true.
Proof.
intros H H1.
assert (F0: (0 < Zpos p)%Z); try (red; auto; fail).
assert (F1: (0 < 2 ^ (Zpos p - 1))%Z); auto with zarith.
case (phi_to_Z s); intros Hs1 Hs2.
generalize (spec_int31i s p H); rewrite H1; intros H2.
generalize (spec_int31i (int31a s p) p H).
case int31i; auto.
unfold int31a.
rewrite phi_add; rewrite phi_shiftl31; rewrite phi_p2t31; 
  try (rewrite phi1; rewrite Zmult_1_l); auto with zarith.
2: apply Zpow_facts.Zpower_lt_monotone; auto with zarith.
rewrite (fun x => Zmod_small x (2^31)%Z).
- generalize (Z_div_plus_full (phi s) 1%Z (2 ^ (Zpos p - 1))%Z).
  ...
  auto with zarith.
- split; auto with zarith.
  replace (phi s) with (2^(Zpos p) * (phi s / 2 ^ (Zpos p)) + (phi s mod (2^ (Zpos p - 1))))%Z.
  + apply Z.lt_le_trans with (2 ^ Zpos p * (phi s / 2 ^ Zpos p) + 2 ^ Zpos p)%Z.
    rewrite <- Zplus_assoc; apply Zplus_lt_compat_l.
    apply Z.le_lt_trans with (2 ^ (Zpos p - 1) - 1 + 2 ^ (Zpos p - 1))%Z.
    * apply Zplus_le_compat_r.
      case (Z_mod_lt (phi s) (2 ^ (Zpos p - 1))); auto with zarith.
    ...
    * replace (2 ^ Zpos p * (phi s / 2 ^ Zpos p) + 2 ^ Zpos p)%Z with
           (2 ^ Zpos p * (phi s / 2 ^ Zpos p + 1))%Z; try ring.
      replace (2 ^ 31)%Z with (2 ^ (Zpos p) * (2^ (31 - Zpos p)))%Z.
      -- apply Zmult_le_compat_l; auto with zarith.
         assert (phi s / 2 ^ Zpos p < 2 ^ (31 - Zpos p))%Z; auto with zarith.
         apply Zdiv_lt_upper_bound; auto with zarith.
         rewrite <- Zpower_exp; auto with zarith.
         replace (31 - Zpos p + Zpos p)%Z with 31%Z; auto; ring.
      -- rewrite <- Zpower_exp; auto with zarith.
         apply f_equal2 with (f := Zpower); auto; ring.
  + apply sym_equal.
    pattern (phi s) at 1; rewrite (Z_div_mod_eq (phi s) (2 ^ Zpos p)%Z).
    * apply f_equal2 with (f := Zplus); auto.
      pattern (phi s) at 1; rewrite (Z_div_mod_eq (phi s) (2 ^ (Zpos p - 1))%Z).
      -- rewrite Zplus_mod.
         replace (2 ^(Zpos p))%Z with (2^(Zpos p - 1) * (2 ^ 1))%Z.
         ++ rewrite Zmult_mod_distr_l; rewrite Zpow_facts.Zpower_1_r; rewrite H2.
            ...
            case (Z_mod_lt (phi s) (2 ^ (Zpos p - 1))%Z); auto with zarith.
         ++ rewrite <- Zpower_exp; auto with zarith.
            apply f_equal2 with (f := Zpower); auto with zarith.
...
Lemma int31aif s p q : (Zpos p <= 31)%Z -> (Zpos q <= 31)%Z ->
  int31i s p = false -> p <> q -> int31i (int31a s p) q = int31i s q.
Proof.
intros Hp Hq H Hd.
assert (F0: (0 < Zpos p)%Z); try (red; auto; fail).
...
assert (F3: (0 < 2 ^ (Zpos q - 1))%Z).
  now apply Z.pow_pos_nonneg; auto with zarith.
generalize (spec_int31i s p Hp); rewrite H; intros H1.
case (phi_to_Z s); intros Hs1 Hs2.
generalize (spec_int31i s q Hq) (spec_int31i (int31a s p) q Hq).
replace ((phi s / 2 ^ (Zpos q - 1)) mod 2)%Z with
        ((phi (int31a s p) / 2 ^ (Zpos q - 1)) mod 2)%Z.
  now intros HH; rewrite HH; case int31i; case int31i; auto;
      intros; discriminate.
unfold int31a.
rewrite phi_add; rewrite phi_shiftl31; rewrite phi_p2t31; 
  try (rewrite phi1; rewrite Zmult_1_l); auto with zarith.
2: apply Zpow_facts.Zpower_lt_monotone; auto with zarith.
assert (F : (phi s = 2 ^ (Zpos p) * (phi s / 2 ^ (Zpos p)) + 
                     (phi s mod (2^ (Zpos p - 1))))%Z).
  pattern (phi s) at 1; rewrite (Z_div_mod_eq (phi s) (2 ^ Zpos p)%Z).
    apply f_equal2 with (f := Zplus); auto.
    pattern (phi s) at 1; rewrite (Z_div_mod_eq (phi s) (2 ^ (Zpos p - 1))%Z).
      rewrite Zplus_mod.
      replace (2 ^(Zpos p))%Z with (2^(Zpos p - 1) * (2 ^ 1))%Z.
        ...
        rewrite Zmult_0_r; rewrite Zplus_0_l.
        rewrite Zmod_mod; apply Zmod_small.
        now case (Z_mod_lt (phi s) (2 ^ (Zpos p - 1))%Z); auto with zarith.
      rewrite <- Zpower_exp; auto with zarith.
      ...
      replace (2 ^ (Zpos q - 1))%Z with (2^(Zpos p + (Zpos q - Zpos p -1)))%Z.
        ...
        rewrite Z_div_plus_full; auto with zarith.
          ...
          now rewrite <- Zpower_exp; auto with zarith; 
              apply f_equal2 with (f := Zpower); auto with zarith.
        now assert (0 < 2^ Zpos p)%Z; auto with zarith.
      now apply f_equal2 with (f := Zpower); auto with zarith.
...
apply Z.lt_le_trans with (2 ^ Zpos p * (phi s / 2 ^ Zpos p) + 2 ^ Zpos p)%Z.
  ...
  apply Z.le_lt_trans with (2 ^ (Zpos p - 1) - 1 + 2 ^ (Zpos p - 1))%Z.
    apply Zplus_le_compat_r.
    now case (Z_mod_lt (phi s) (2 ^ (Zpos p - 1))); auto with zarith.
  replace (2 ^Zpos p)%Z with (2^(Zpos p -1) * 2 ^1)%Z.
    now rewrite Zpow_facts.Zpower_1_r; auto with zarith.
...
replace (2 ^31)%Z with (2 ^ (Zpos p) * (2^(31 - Zpos p)))%Z.
  ...
  apply Zdiv_lt_upper_bound; auto with zarith.
  rewrite <- Zpower_exp; auto with zarith.
  now replace (31 - Zpos p + Zpos p)%Z with 31%Z; auto; ring.
rewrite <- Zpower_exp; auto with zarith.
apply f_equal2 with (f := Zpower); auto; ring.
Qed.

Lemma int31_0: forall p, (Zpos p < 31)%Z -> int31i 0 p = false.
Proof.
intros p Hp.
generalize (spec_int31i 0 p).
rewrite phi0; rewrite Zdiv_0_l; rewrite Zmod_0_l.
case int31i; auto; intros HH2.
absurd (0 = 1)%Z; auto with zarith.
Qed.

Lemma tc7ai: forall s l p, validtc7 l -> valid31 p ->
  tc7i l s p = false -> tc7i l (tc7a l s p) p = true.
Proof.
intros s l p H Hp.
assert (Hp1: (Zpos p < 31)%Z); auto.
case (validtc7_inv _ H); clear H.
intros p1 [H1 [H2 | [H2 | [H2 | [H2 | [H2 | [H2 | H2]]]]]]]; rewrite H1 , H2;
  case s; simpl; intros; apply int31ai; auto with zarith; apply int31_0; auto.
Qed.

Lemma tc7aif: forall s l1 l2 p1 p2, validtc7 l1 -> validtc7 l2  -> 
  valid31 p1 -> valid31 p2 ->
...
assert (Hpp1: (Zpos p1 < 31)%Z); auto.
assert (Hpp2: (Zpos p2 < 31)%Z); auto.
intros c1 [H1 [H2 | [H2 | [H2 | [H2 | [H2 | [H2 | H2]]]]]]]; rewrite H1;
  ...
  intros c2 [H1 [H2 | [H2 | [H2 | [H2 | [H2 | [H2 | H2]]]]]]]; rewrite H1;
    ...
    try (intros; apply int31_0; auto with zarith);
    ((intros i1 i2 i3 i4 i5 i6 i7 H1 [HH | HH]) || 
     (intros _ [HH | HH]));
    try match goal with H : ?X <> ?X |- _ => case H; trivial end;
    rewrite ?int31aif; auto with zarith;
    apply int31_0; auto.
Qed.

Lemma tc4ai s l p : validtc4 l -> valid31 p ->
...
Lemma tc3ai s l p : validtc3 l -> valid31 p ->
...
Lemma to1ai s l l1 p : validto1 l -> validtc3 l1 -> valid31 p ->
...
Lemma to1aif s l1 l2 l3 l4 p1 p2 : validto1 l1 -> validto1 l2  ->
  validtc3 l3 -> valid31 p1 -> validtc3 l4 -> valid31 p2 ->
...
Lemma to2ai s l l1 p : validto2 l -> validtc3 l1 -> valid31 p ->
...
Lemma to3ai s l l1 p : validto3 l -> validtc3 l1 -> valid31 p ->
...
Lemma to4ai s l l1 p : validto4 l -> validtc3 l1 -> valid31 p ->
...
Lemma to5ai s l l1 p : validto5 l -> validtc3 l1 -> valid31 p ->
...
Lemma to6ai s l l1 p : validto6 l -> validtc3 l1 -> valid31 p ->
...
Lemma tc2ai s l l1 p : validtc2 l -> validto6 l1 -> valid31 p ->
  tc2i l l1 p s = false -> tc2i l l1 p (tc2a l l1 p s) = true.
Proof.
intros Hl Hl1 Hp.
assert (Hp1: (Zpos p < 31)%Z); auto.
case (validtc2_inv _ Hl); clear Hl.
intros p1 [l2  [H1 [[H2 | H2] H3]]]; rewrite H1;
  rewrite H2; clear H1 H2; case s; unfold tc2i, tc2a; auto;
  intros; apply to6ai; auto.
Qed.

Lemma tc2aif s l1 l2 l3 l4 p1 p2 : validtc2 l1 -> validtc2 l2  ->
  validto6 l3 -> valid31 p1 -> validto6 l4 -> valid31 p2 ->
  tc2i l1 l3 p1 s = false -> (l1 <> l2 \/ l3 <> l4 \/ p1 <> p2) -> 
  tc2i l2 l4 p2 (tc2a l1 l3 p1 s) = tc2i l2 l4 p2 s.
Proof.
intros Hl1 Hl2 Hl3 Hl4 Hp1 Hp2.
case (validtc2_inv _ Hl1); clear Hl1;
  case (validtc2_inv _ Hl2); clear Hl2.
assert (Hpp1: (Zpos p1 < 31)%Z); auto.
assert (Hpp2: (Zpos p2 < 31)%Z); auto.
intros c1 [l5 [H1 [[H2 | H2] Hp]]]; rewrite H1;
  rewrite H2; clear H1 H2; 
intros q1 [l6 [H1 [[H2 | H2] Hq]]]; rewrite H1;
  rewrite H2; clear H1 H2; case s; unfold tc2i, tc2a; auto;
  intros; rewrite to6aif; auto;
repeat match goal with H : _ \/ _ |- _  => 
  case H; clear H; intros H1 end; auto;
  right; left; intros HH; case H1; subst; auto.
Qed.
...
(** we first show the equivalence of int31 with a naive                     **)
(** version that is easier to manipulate in proofs                          **)
Fixpoint naive_int31it_aux (m : nat) (p : positive) 
                     (c : int31) (l1 : list cube) 
                     (l2 : list orientation)
                     (a : A) {struct m} :=
 let a1 := if int31i c p then next a l1 l2 p else a in
 match m with 
 | O => a1
 | S m1 => naive_int31it_aux m1 (Pos.succ p) c l1 l2 a1
 end.

Lemma naive_aux0 p c l1 l2 a :
  naive_int31it_aux 0 p c l1 l2 a = if int31i c p then next a l1 l2 p else a.
Proof. auto. Qed.

Lemma naive_auxS m p c l1 l2 a :
  naive_int31it_aux (S m) p c l1 l2 a = 
  naive_int31it_aux m (Pos.succ p) c l1 l2 
   (if int31i c p then next a l1 l2 p else a).
Proof.
unfold naive_int31it_aux; auto.
Qed.

Definition naive_int31it l1 l2 a c := naive_int31it_aux 29 1 c l1 l2 a.

Lemma phi_compare31_compat u1 u2 v1 v2 :
  phi u1 = phi u2 -> phi v1 = phi v2 ->  u1 ?= v1 = (u2 ?= v2). 
Proof.
intros Hu1 Hv1.
generalize (phi_compare31 u1 v1) (phi_compare31 u2 v2);
  do 2 case compare31; auto; intros H1 H2; contradict H2;
  auto with zarith.
Qed.

Theorem int31it_naive a l1 l2 c :
  int31it _ next l1 l2 a c = naive_int31it l1 l2 a c.
Proof.
revert a l1 l2 c.
assert (F:  forall m p c c1 tc2 l1 l2 a,
  (Z_of_nat m + Zpos p = 30)%Z -> 
  phi c1 = phi (shiftr31 c (p2t31 p)) ->
  phi tc2 = phi (shiftr31 c (p2t31 (Pos.succ p))) ->
     int31it_aux _ next (S m) p c1 tc2 l1 l2  a = naive_int31it_aux m p c l1 l2 a).
  intros m; elim m; clear m; auto.
  intros p c c1 cc2 l1 l2 a H1 H2 H3.
    unfold int31it_aux, naive_int31it_aux, int31i.
    assert (F0: (0 < Zpos p)%Z); try (red; auto; fail).
    unfold Z_of_nat in H1.
    match goal with |- context[compare31 ?X  ?Y] =>
         set (u := X ?= Y)
    end.
    match goal with |- context[compare31 ?X  ?Y] =>
         set (v := compare31 X Y);
      ...
      now case u; auto.
    unfold u; unfold v; apply phi_compare31_compat; auto.
    repeat rewrite phi_shiftl31; rewrite phi1; auto with zarith;
    try rewrite H3; rewrite phi_shiftr31; auto with zarith;
    repeat rewrite phi_shiftr31; repeat rewrite phi_p2t31; 
    ...
    auto with zarith.
      ...
      now apply f_equal2 with (f := Zpower); auto with zarith.
      rewrite Zdiv_Zdiv; auto with zarith.
      replace (2^31)%Z with (2^30 * 2 ^1)%Z; auto.
      apply Zmult_lt_compat_r; auto with zarith.
      rewrite <- Zpower_exp; auto with zarith.
      apply Zdiv_lt_upper_bound; auto with zarith.
      case (phi_to_Z c); intros HH1 HH2; auto.
      rewrite <- Zpower_exp; auto with zarith.
      apply Z.lt_le_trans with (2^31)%Z.
  ...
  simpl int31it_aux; rewrite Hk.
  rewrite naive_auxS.
  assert (F0: (0 < Zpos p)%Z); try (red; auto; fail).
  rewrite inj_S in H1; unfold int31i.
  match goal with |- context[compare31 ?X  ?Y] =>
    set (u := X ?= Y)
  end.
  match goal with |- context[compare31 ?X  ?Y] =>
    ...
    now case u; (apply Hrec; auto; [
        ...
        |  
         repeat rewrite phi_shiftr31; try rewrite phi1; auto with zarith;
         rewrite phi_p2t31; repeat rewrite Zpos_succ_morphism; auto with zarith;
         rewrite H3; repeat rewrite phi_p2t31;
         repeat rewrite Zpos_succ_morphism; auto with zarith;
         rewrite phi_shiftr31; repeat rewrite phi_p2t31;
         repeat rewrite Zpos_succ_morphism; auto with zarith;
         rewrite Zdiv_Zdiv; auto with zarith;
         rewrite <- Zpower_exp; auto with zarith;
         apply f_equal2 with (f := Z.div); auto;
  ...
  apply Z.lt_le_trans with (2 ^ 31)%Z.
    now case (phi_to_Z c); auto with zarith.
  now apply Zpow_facts.Zpower_le_monotone; auto with zarith.
intros a l1 l2 c; unfold int31it, naive_int31it.
apply F; auto with zarith.
rewrite phi_shiftr31; auto with zarith.
Qed.
...
Theorem make_list31i l1 l2 c k ll :
  In k (int31it _ make_list l1 l2 ll c) ->
   In k ll \/
   exists p, (Zpos p < 31)%Z /\ k = (l1,l2,p) /\ int31i c p = true.
Proof.
rewrite int31it_naive; 
  unfold naive_int31it; generalize l1 l2 c k ll; clear l1 l2 c k ll.
assert(F:
  forall m p l1 l2 c k ll,
  (Z_of_nat m + Zpos p = 30)%Z -> 
  In k
   (naive_int31it_aux _ make_list  m p c l1 l2 ll) ->
In k ll \/
exists p1 : positive, (Zpos p <= Zpos p1 < 31)%Z /\ 
                      k = (l1,l2,p1) /\ int31i c p1 = true).
- intros m; elim m; clear m.
  + intros p l1 l2 c k ll1 H.
    rewrite naive_aux0.
    case_eq (int31i c p); intros Heq Hf; auto.
  ...
  + intros m Hrec p l1 l2 c k ll H.
    rewrite naive_auxS.
    assert (F1: (Z_of_nat  m + (Zpos (Pos.succ p)) = 30)%Z).
      rewrite <- H.
      now rewrite inj_S; rewrite Zpos_succ_morphism; auto with zarith.
    case_eq (int31i c p); intros H1.
...
- intros l1 l2 c k ll H.
  case (F 29%nat 1%positive l1 l2 c k ll).
  + reflexivity.
  + exact H.
...
Theorem make_list31irI k l1 l2 ll c :
  In k ll -> In k (int31it _ make_list l1 l2 ll c).
Proof.
intro H; rewrite int31it_naive; unfold naive_int31it;
  replace ll with (ll ++ nil); try rewrite naive_auxA; auto with datatypes.
Qed.

Theorem make_list31irA c l1 l2 ll1 ll2 :
   int31it _ make_list l1 l2 (ll1++ll2) c =
   ll1 ++ int31it _ make_list l1 l2 ll2 c.
Proof.
repeat rewrite int31it_naive.
unfold naive_int31it.
rewrite naive_auxA.
reflexivity.
Qed.

Theorem make_list31ir l1 l2 c p ll :
  (Zpos p < 31)%Z -> int31i c p = true ->
  In (l1,l2,p) (int31it _ make_list l1 l2 ll c).
Proof.
rewrite int31it_naive; 
  unfold naive_int31it; generalize l1 l2 c p ll; clear l1 l2 c p ll.
assert(F:
  forall m p l1 l2 c ll p1,
  (Z_of_nat m + Zpos p = 30)%Z -> 
    (Zpos p <= Zpos p1 < 31)%Z -> int31i c p1 = true ->
  In (l1,l2,p1)
   (naive_int31it_aux _
  ...
  replace p with p1.
    simpl naive_int31it_aux; rewrite H2.
...
Theorem make_list31fold l1 l2 a c :
  int31it _ next l1 l2 a c =
  fold_left nextp (int31it _ make_list l1 l2 nil c) a.
Proof.
rewrite int31it_naive; unfold make_list.
  rewrite int31it_naive; unfold naive_int31it.
generalize l1 l2 a c; clear l1 l2 a c.
generalize (1%positive); elim 29%nat.
  intros p l1 l2 a c; repeat rewrite naive_aux0.
  now case int31i; auto.
...
Theorem make_listtc7i l1 l2 c k ll :
   ...
   In k ll \/(exists l3, exists p, k = (l3 ++ l1,l2,p) 
               /\ validtc7 (rev l3) /\ valid31 p 
                  /\ tc7i (rev l3) c p = true).
Proof.
revert ll; unfold tc7it; case c.
  now intros ll HH; left; assumption.
intros cc1 cc2 cc3 cc4 cc5 cc6 cc7 ll.
set (ll1 := (int31it _ make_list (C7 :: l1) l2 ll cc7)).
set (ll2 := (int31it _ make_list (C6 :: l1) l2 ll1 cc6)).
set (ll3 := (int31it _ make_list (C5 :: l1) l2 ll2 cc5)).
set (ll4 := (int31it _ make_list (C4 :: l1) l2 ll3 cc4)).
set (ll5 := (int31it _ make_list (C3 :: l1) l2 ll4 cc3)).
set (ll6 := (int31it _ make_list (C2 :: l1) l2 ll5 cc2)).
intros H.
case (make_list31i _ _ _ _ _ H); clear H; unfold ll6; intros H.
- case (make_list31i _ _ _ _ _ H); clear H; unfold ll5; intros H.
  + case (make_list31i _ _ _ _ _ H); clear H; unfold ll4; intros H.
    * case (make_list31i _ _ _ _ _ H); clear H; unfold ll3; intros H.
      -- case (make_list31i _ _ _ _ _ H); clear H; unfold ll2; intros H.
         ++ case (make_list31i _ _ _ _ _ H); clear H; unfold ll1; intros H.
            ** case (make_list31i _ _ _ _ _ H); clear H; intros H.
               --- left; assumption.
               --- case H; intros p (Hp1, (Hp2, Hp3)).
                   ...
                   exists (C7 :: nil); exists p; split.
                     now reflexivity.
                   split.
                     now unfold validtc7, valid31, rev, app; auto.
                     now split; auto.
            ** case H; intros p (Hp1, (Hp2, Hp3)).
               right; rewrite Hp2.
...
intros cc1 cc2 cc3 cc4 cc5 cc6 cc7; repeat rewrite make_list31irA.
reflexivity.
Qed.
...
Theorem make_listtc7ir l p c l1 l2 ll :
   validtc7 (rev l) -> valid31 p -> tc7i (rev l) c p = true -> 
...
case HH1; [intros HH2 | intros [HH2 | [HH2 | [HH2 | [HH2 | [HH2 | HH2]]]]]];
  ...
  unfold tc7it.
- apply make_list31ir; auto.
- apply  make_list31irI; apply make_list31ir; auto.
- do 2 (apply  make_list31irI); apply make_list31ir; auto.
- do 3 (apply  make_list31irI); apply make_list31ir; auto.
- do 4 (apply  make_list31irI); apply make_list31ir; auto.
- do 5 (apply  make_list31irI); apply make_list31ir; auto.
- do 6 (apply  make_list31irI); apply make_list31ir; auto.
Qed.
...
repeat rewrite make_list31fold.
repeat rewrite <- fold_left_app.
repeat rewrite <- app_ass.
repeat rewrite <- make_list31irA.
...
- case (make_listtc7i _ _ _ _ _ H); clear H; unfold ll3; intros H.
  + case (make_listtc7i _ _ _ _ _ H); clear H; unfold ll2; intros H.
    * case (make_listtc7i _ _ _ _ _ H); clear H; unfold ll1; intros H.
      ...
      split.
        now rewrite rev_unit; unfold validtc4, valid31; auto.
      split; auto.
...
Theorem make_listtc4ir l p c l1 l2 ll :
   validtc4 (rev l) ->  valid31 p -> tc4i (rev l) c p = true -> 
  ...
  + case (make_listtc4i _ _ _ _ _ H); clear H; unfold ll2; intros H.
    * case (make_listtc4i _ _ _ _ _ H); clear H; unfold ll1; intros H.
      ...
      exists (l3 ++ (C3 :: nil)); exists p; split.
        now rewrite app_ass; reflexivity.
      split.
        now rewrite rev_unit; unfold validtc3, valid31; auto.
      split; auto.
...
Theorem make_listtc3ir l p c l1 l2 ll :
   validtc3 (rev l) ->  valid31 p -> tc3i (rev l) c p = true -> 
...
Theorem make_listto1i l1 l2 c k ll :
   ...
   In k ll \/(exists l3, exists l4, exists p, k = (l3++l1,l4++l2,p) 
               /\ validtc3 (rev l3) /\ validto1 (rev l4) /\ valid31 p 
...
Theorem make_listto1ir l1 l2 p c l3 l4 ll :
   validtc3 (rev l1) -> validto1 (rev l2) ->  valid31 p -> 
...
Theorem make_listto2i l1 l2 c k ll :
   ...
   In k ll \/(exists l3, exists l4, exists p, k = (l3++l1,l4++l2,p) 
               /\ validtc3 (rev l3) /\ validto2 (rev l4) /\ valid31 p 
...
Theorem make_listto2ir l1 l2 p c l3 l4 ll :
   validtc3 (rev l1) -> validto2 (rev l2) ->  valid31 p -> 
...
Theorem make_listto3i l1 l2 c k ll :
   ...
   In k ll \/(exists l3, exists l4, exists p, k = (l3++l1,l4++l2,p) 
               /\ validtc3 (rev l3) /\ validto3 (rev l4) /\ valid31 p 
...
Theorem make_listto3ir l1 l2 p c l3 l4 ll :
   validtc3 (rev l1) -> validto3 (rev l2) ->  valid31 p -> 
...
Theorem make_listto4i l1 l2 c k ll :
   ...
   In k ll \/(exists l3, exists l4, exists p, k = (l3++l1,l4++l2,p) 
               /\ validtc3 (rev l3) /\ validto4 (rev l4) /\ valid31 p 
...
Theorem make_listto4ir l1 l2 p c l3 l4 ll :
   validtc3 (rev l1) -> validto4 (rev l2) ->  valid31 p -> 
...
Theorem make_listto5i l1 l2 c k ll :
   ...
   In k ll \/(exists l3, exists l4, exists p, k = (l3++l1,l4++l2,p) 
               /\ validtc3 (rev l3) /\ validto5 (rev l4) /\ valid31 p 
...
Theorem make_listto5ir l1 l2 p c l3 l4 ll :
   validtc3 (rev l1) -> validto5 (rev l2) ->  valid31 p -> 
...
Theorem make_listto6i l1 l2 c k ll :
   In k (to6it _ make_list l1 l2 ll c) -> 
   In k ll \/(exists l3, exists l4, exists p, k = (l3++l1,l4++l2,p) 
               /\ validtc3 (rev l3) /\ validto6 (rev l4) /\ valid31 p 
                  /\ to6i (rev l4) c (rev l3) p = true).
Proof.
unfold to6it; case c.
  now intro HH; left; assumption.
intros cc1 cc2 cc3.
set (ll1 := (to5it _ make_list l1 (O3 :: l2) ll cc3)).
set (ll2 := (to5it _ make_list l1 (O2 :: l2) ll1 cc2)).
set (ll3 := (to5it _ make_list l1 (O1 :: l2) ll2 cc1)).
intros H.
...
Theorem make_listto6irA c l1 l2 ll1 ll2 :
   to6it _ make_list l1 l2 (ll1++ll2) c =
   ll1 ++ to6it _ make_list l1 l2 ll2 c.
Proof.
case c; unfold to6it; auto with datatypes.
intros cc1 cc2 cc3; repeat rewrite make_listto5irA; auto.
Qed.

Theorem make_listto6irI k c l1 l2 ll :
   In k ll -> In k (to6it _ make_list l1 l2 ll c).
Proof.
intro H; replace ll with (ll ++ nil);
 try rewrite make_listto6irA; auto with datatypes.
Qed.

Theorem make_listto6ir l1 l2 p c l3 l4 ll :
   validtc3 (rev l1) -> validto6 (rev l2) ->  valid31 p -> 
   to6i (rev l2) c (rev l1) p = true -> 
   In (l1++l3,l2++l4,p) (to6it _ make_list l3 l4 ll c).
Proof.
case c.
...
Theorem make_listto6fold l1 l2 a c :
  to6it _ next l1 l2 a c =
  fold_left nextp (to6it _ make_list l1 l2 nil c) a.
Proof.
case c.
  now reflexivity.
...
Theorem make_listtc2i c k ll :
   In k (tc2it _ make_list ll c) -> 
   In k ll \/(exists l3, exists l4, exists p, k = (l3,l4,p) 
               /\ validtc2 (rev l3) /\ validto6 (rev l4) /\ valid31 p 
                  /\ tc2i (rev l3) (rev l4) p c = true).
Proof.
unfold tc2it; case c.
  now intros HH; left; assumption.
intros cc1 cc2.
set (ll1 := (to6it _ make_list (C2 :: nil) nil ll cc2)).
set (ll2 := (to6it _ make_list (C1 :: nil) nil ll1 cc1)).
intros H.
case (make_listto6i _ _ _ _ _ H); clear H; unfold ll2; intros H.
- case (make_listto6i _ _ _ _ _ H); clear H; unfold ll1; intros H.
    now left; assumption.
  case H; intros l3 (l4, (p, (Hp1, (Hp2, (Hp3, (Hp4, Hp5)))))).
  right; rewrite Hp1.
  ...
  split.
  rewrite rev_unit; unfold validtc2, valid31; auto.
  split; [idtac | split]; auto.
  rewrite rev_unit; unfold tc2i; exact Hp5.
- case H; intros l3 (l4, (p, (Hp1, (Hp2, (Hp3, (Hp4, Hp5)))))).
  right; rewrite Hp1.
...
Theorem make_listtc2irA c ll1 ll2 :
   tc2it _ make_list (ll1++ll2) c = ll1 ++ tc2it _ make_list ll2 c.
Proof.
case c; unfold tc2it; auto with datatypes.
intros cc1 cc2; repeat rewrite make_listto6irA; auto.
Qed.

Theorem make_listtc2irI k c ll :
   In k ll -> In k (tc2it _ make_list ll c).
Proof.
intro H; replace ll with (ll ++ nil);
 try rewrite make_listtc2irA; auto with datatypes.
Qed.

Theorem make_listtc2ir l1 l2 p c ll :
   validtc2 (rev l1) -> validto6 (rev l2) ->  valid31 p ->
   tc2i (rev l1) (rev l2) p c = true -> 
   In (l1,l2,p) (tc2it _ make_list ll c).
Proof.
case c.
now unfold tc2i; intros _ _ _ HH; discriminate.
intros cc1 cc2 Hl1 Hl2 Hp.
case (validtc2_inv _ Hl1); clear Hl1.
intros p1 (l3, (HH, (HH1, Hl3))).
rewrite HH.
case HH1; intros HH2;
  ...
  now apply  make_listto6ir; auto; rewrite rev_involutive; auto.
apply  make_listto6irI; apply make_listto6ir; auto; rewrite rev_involutive; auto.
Qed.

Theorem make_listtc2fold a c :
  tc2it _ next a c = fold_left nextp (tc2it _ make_list nil c) a.
Proof.
case c.
  now reflexivity.
intros cc1 cc2; unfold tc2it.
repeat rewrite make_listto6fold.
repeat rewrite <- fold_left_app.
repeat rewrite <- app_ass.
repeat rewrite <- make_listto6irA.
repeat rewrite <- app_nil_end.
reflexivity.
Qed.
...
Lemma check31_all c : int31all c = true -> 
  forall s, (Zpos s < 31)%Z -> int31i c s = true.
Proof.
unfold int31all.
generalize (phi_compare31 c 1073741823); case compare31;
  try (intros; discriminate).
intros H1 _ s Hs.
assert (F0: (Zpos s <= 31)%Z); auto with zarith.
generalize (spec_int31i c s F0); rewrite H1.
case int31i; auto.
change (phi 1073741823)%Z with (1073741823)%Z.
generalize s Hs; clear s Hs F0.
do 4 (intros s; case s; clear s;
...
Lemma checktc7_all : forall c, tc7all c = true -> 
  forall l p, validtc7 l -> valid31 p -> tc7i l c p = true.
Proof.
...
intros c1 (Hc1, [H2 | [H2 |  [H2 | [H2 | [H2 | [H2 | H2]]]]]]); 
  ...
  intros Ha Hp;
  match goal with |-  int31i ?X  _ = _ => 
    apply (fun x => check31_all X x _ Hp); generalize Ha
  end;  do 7 (case int31all; auto).
Qed.

Lemma checktc4_all c l p : tc4all c = true -> 
  validtc4 l -> valid31 p -> tc4i l c p = true.
Proof.
...
Lemma checktc3_all c l p : tc3all c = true -> 
  validtc3 l -> valid31 p -> tc3i l c p = true.
Proof.
case c; [intros HH; discriminate | idtac].
...
Lemma checkto1_all c l1 l2 p : to1all c = true -> 
  validtc3 l1 -> validto1 l2 -> valid31 p ->
  to1i l2 c l1 p = true.
Proof.
case c; [intros HH; discriminate | idtac].
intros p1 p2 p3 Hc Hl; simpl in Hc; generalize Hc;
...
Lemma checkto2_all c l1 l2 p : to2all c = true -> 
  validtc3 l1 -> validto2 l2 -> valid31 p ->
  to2i l2 c l1 p = true.
Proof.
case c; [intros HH; discriminate | idtac].
intros p1 p2 p3 Hc Hl; simpl in Hc; generalize Hc;
...
Lemma checkto3_all c l1 l2 p : to3all c = true -> 
  validtc3 l1 -> validto3 l2 -> valid31 p ->
  to3i l2 c l1 p = true.
Proof.
case c; [intros HH; discriminate | idtac].
intros p1 p2 p3 Hc Hl; simpl in Hc; generalize Hc;
...
Lemma checkto4_all c l1 l2 p : to4all c = true -> 
  validtc3 l1 -> validto4 l2 -> valid31 p ->
  to4i l2 c l1 p = true.
Proof.
case c; [intros HH; discriminate | idtac].
intros p1 p2 p3 Hc Hl; simpl in Hc; generalize Hc;
...
Lemma checkto5_all c l1 l2 p : to5all c = true -> 
  validtc3 l1 -> validto5 l2 -> valid31 p ->
  to5i l2 c l1 p = true.
Proof.
case c; [intros HH; discriminate | idtac].
intros p1 p2 p3 Hc Hl; simpl in Hc; generalize Hc;
...
Lemma checkto6_all c l1 l2 p : to6all c = true -> 
  validtc3 l1 -> validto6 l2 -> valid31 p ->
  to6i l2 c l1 p = true.
Proof.
case c; [intros HH; discriminate | idtac].
intros p1 p2 p3 Hc Hl; simpl in Hc; generalize Hc;
...
Lemma checktc2_all c l1 l2 p : tc2all c = true -> 
     validtc2 l1 -> validto6 l2 -> valid31 p ->
     tc2i l1 l2 p c = true.
Proof.
case c; [intros HH; discriminate | idtac].
intros p1 p2 Hc Hl; simpl in Hc; generalize Hc;
  clear Hc; case (validtc2_inv _ Hl).
intros c1 [l3 [Hl1 [[H2 | H2] Hl2]]];
  rewrite Hl1; rewrite H2; unfold tc3i;
  intros Hp; apply checkto6_all; auto;
  generalize Hp; do 3 (case to6all; auto).
Qed.
...
Lemma encode_length n l : 
  length l <= n -> length (encode l n) = length l.
Proof.
...
Fixpoint decrease l m {struct l} := 
  match l with 
    nil => True
  | n ::  l1 => (c2N n < m)%nat /\ decrease l1 (pred m)
  end.
...
rewrite encode_max; auto.
  ...
  generalize Hp; case l; simpl length; simpl app; clear Hp.
    ...
    rewrite cube_encode_decode; auto with arith.
...
rewrite encode_max; auto.
  ...
  generalize Hp; case l; simpl length; simpl app; clear Hp.
    intros Hp _ (Hd1, (Hd2, (Hd3, (Hd4, (Hd5, (Hd6, _)))))).
    repeat split; auto.
    now unfold valid31; generalize Hd2 Hd3; case p5; case p6; simpl;
        intros H1 H2; red; auto;
        contradict H1; auto with arith;
...
Definition tc2i3 l ll :=
  match l with (l1,l2,p) => tc2i l1 l2 p ll end.

Definition tc2a3 l ll :=
  match l with (l1,l2,p) => tc2a l1 l2 p ll end.

Lemma make_listtc2it3 c k ll :
  In k (tc2it _ make_list ll c) ->
  In k ll \/ (valid3 (rev3 k) /\ tc2i3 (rev3 k) c = true).
Proof.
intro F; case (make_listtc2i _ _ _ F); auto.
intros (l3, (l4, (p, (Hp, (Hp1, (Hp2, (Hp3, Hp4))))))).
right; rewrite Hp; repeat split; auto.
Qed.

Lemma tc2ai3 l ll :
  tc2i3 l ll = false -> valid3 l -> tc2i3 l (tc2a3 l ll) = true.
Proof.
...
Lemma tc2aif3 l1 l2 ll :
  tc2i3 l2 ll = false -> valid3 l1 -> valid3 l2 ->
  l1 <> l2 ->
  tc2i3 l1 (tc2a3 l2 ll) =  tc2i3 l1 ll.
Proof.
...
Lemma make_listtc2ir3 c l ll :
  valid3 (rev3 l) -> tc2i3 (rev3 l) c = true -> 
  In l  (tc2it _ make_list ll c).
Proof.
destruct l as ((l1,l2),p1); simpl.
intros (H1, (H2, H3)) H5.
apply make_listtc2ir; auto.
Qed.

Lemma states_equiv_tc2a s ss ll :
  valid_state s ->
  (forall s : state, In s ss -> valid_state s) ->
  states_equiv ss ll -> tc2i3 (encode_state s) ll = false ->
   states_equiv (s :: ss) (tc2a3 (encode_state s) ll).
Proof.
intros H H1 H2 H3 l; split; intros H4.
- simpl.
  case (make_listtc2it3 _ _ _ H4).
  ...
  rewrite tc2aif3; auto.
    intros H7; right; unfold states_equiv in H2; rewrite <-H2.
    now apply make_listtc2ir3; auto.
  apply encode_valid; auto.
- case (list3_dec (rev3 l) (encode_state s)); intros H6.
    apply make_listtc2ir3; rewrite H6.
      now apply encode_valid; auto.
    now apply tc2ai3; auto; apply encode_valid; auto.
  ...
  case (make_listtc2it3 _ _ _ H4); auto.
    now intros HH; inversion HH.
  intros (H7, H8).
  apply make_listtc2ir3; auto.
  rewrite tc2aif3; auto.
  apply encode_valid; auto.
Qed.

Definition filter (s : tc2 * tc2) l := 
  let (states, nstates) := s in
  let ll := encode_state l in
  if tc2i3 ll states then s else
  let states1 := tc2a3 ll states in
  let nstates1 := tc2a3 ll nstates in (states1, nstates1).

Lemma in_states_equiv ss ll s :
  valid_state s ->
  (forall s, In s ss -> valid_state s) ->
  states_equiv ss ll ->
  in_states s ss = tc2i3 (encode_state s) ll.
Proof.
intros Hg Hg1 H.
...
case_eq (tc2i3 (encode_state s) ll1); simpl fst; simpl snd; auto.
split; apply states_equiv_tc2a; auto.
generalize H.
rewrite  <- in_states_equiv with (3 := H1); auto.
...
case (fold_next_equiv ss1 ss2 nil ll1 ll2 OTC2); auto with datatypes.
...
- case (fold_nexts_valid (ss1, ss2) nil); simpl; auto.
  ...
  intros ss3 ss4.
  match goal with |- context[tc2it _ ?X ?Y ?Z] =>
    case (tc2it _ X Y Z)
  end.
...
split; red; simpl fst; simpl snd.
  replace (tc2it _ make_list nil s0) with
...
Lemma iter_true_reachable n :
  (forall s, valid_state s -> tc2i3 (encode_state s) (fst (iter n)) = true 
        -> nlreachable n s).
Proof.
...
Lemma iter_final n :
  let (s1,s2) := iter n in
   match s2 with 
    OTC2 => forall s, reachable s -> nlreachable (pred n) s
   | _  => True
   end.
Proof.
...
simpl tc2it; rewrite in_map_iff; intros [_ H2].
...
Definition filter2 pmod (s : (tc2 * tc2) * tc2) l := 
  let ll := encode_state l in
  let (ps, ns) := s in
  let (s1, s2) := ps in
    if (tc2i3 ll s1 || tc2i3 ll s2)%bool
    then s
    else match pmod with
         | xH => ((tc2a3 ll s1,s2), tc2a3 ll ns)
         | xO _ => ((s1, tc2a3 ll s2), tc2a3 ll ns)
         | xI _ => ((tc2a3 ll s1, tc2a3 ll s2), tc2a3 ll ns)
         end. 
...
case_eq (tc2i3 (encode_state s) ll1); simpl fst; simpl snd; auto.
rewrite in_states_equiv with (3 := H2); auto.
case_eq (tc2i3 (encode_state s) ll2); simpl fst; simpl snd; auto.
case p.
- intros _ H4 H5; split.
    now simpl; try apply states_equiv_tc2a; auto.
  split.
    now simpl; try apply states_equiv_tc2a; auto.
  simpl; try apply states_equiv_tc2a; auto.
  generalize H4 H5.
  rewrite  <- in_states_equiv with (3 := H1); auto.
  rewrite  <- in_states_equiv with (3 := H2); auto.
...
Fixpoint iter2_aux n p (ps: (tc2 * tc2) * tc2) := 
  match n with 
   O => ps 
 | S n1 => let (m,p1) := ps in 
           iter2_aux n1 (pos_up p) (tc2it _ (next2p p) (m,OTC2) p1)
  end.
...
case (fold_next2_equiv p ss1 ss2 nil ll1 ll2 OTC2); auto with datatypes.
...
Definition iter2 n := iter2_aux n 2%positive (s0, OTC2, s0).

Lemma iter2_equiv n : tstates_equiv (iter2s n) (iter2 n).
Proof.
unfold iter2s, iter2.
assert (F: tc2it _ make_list nil s0 =
        (map (fun x => rev3 (encode_state x)) (init_state :: nil))).
  now apply refl_equal.
...
Lemma iter2_final n :
  ...
  let (s1,s2) := ps in
   match s3 with 
    OTC2 =>
  (forall s, reachable s -> tc2i3 (encode_state s) s1 = true \/ 
                            tc2i3 (encode_state s)  s2 = true) /\
  (forall s, valid_state s -> tc2i3 (encode_state s) s1 = true -> 
     exists m1, m1 <= n /\ nsreachable m1 s /\ is_odd (porder m1)) /\
  (forall s, valid_state s -> tc2i3 (encode_state s) s2 = true ->
...
intros (l1, l2) l3; case l3; auto.
  ...
  split; [idtac | split].
    intros s Hs; case (H3 _ Hs); intros Hs1.
      ...
      case (make_listtc2it3 _ _ _ F1); auto.
      rewrite rev3_involutive; auto.
  ...
  intros s Hg Hs.
    ...
    generalize (make_listtc2ir3 _ _ nil F1 Hs).
...
simpl tc2it; rewrite in_map_iff; intros [_ HH].
case HH; exists s; simpl; auto.
Qed.

(* A transcription of tc2 of the solver *)
Definition tc2_get_number s s1 s2 :=
  let l := encode_state s in
  if tc2i3 l s1 then
    if tc2i3 l s2 then 3%positive else 1%positive
  else
    if tc2i3 l s2 then 2%positive else 4%positive.

Fixpoint tc2_get_next_aux n s s1 s2 l :=
  match l with
  | nil => None
  | a :: l1 => if positive_eq n (tc2_get_number (m2f a s) s1 s2) then Some a
             else tc2_get_next_aux n s s1 s2 l1
  end.

Definition tc2_get_next s s1 s2 := tc2_get_next_aux (pos_down (tc2_get_number s s1 s2)) s s1 s2 Movel.


Definition tc2_step f s s1 s2 :=
    match tc2_get_next s s1 s2 return list moves with
             None => nil
            | Some a => a ::  f (m2f a s) s1 s2 
            end.

Fixpoint tc2_solve n s s1 s2 :=
  match n with
   O => nil
  | S n1 => tc2_step (tc2_solve n1) s s1 s2 
  end.

Lemma tc2_get_number_equiv s s1 s2 l1 l2 :
  valid_state s -> valid_pair (s1, s2) ->
  pstates_equiv (s1,s2) (l1,l2) ->
    get_number s s1 s2 = tc2_get_number s l1 l2.
Proof.
intros Hg (Hg1, Hg2) (H1, H2); simpl in Hg1, Hg2, H1, H2.
unfold get_number, tc2_get_number.
rewrite (in_states_equiv s1 l1); auto.
rewrite (in_states_equiv s2 l2); auto.
Qed.

Lemma tc2_get_next_aux_equiv n s s1 s2 l1 l2 l :
  valid_state s -> valid_pair (s1, s2) ->
  pstates_equiv (s1,s2) (l1,l2) -> incl l Movel  ->
    get_next_aux n s s1 s2 l = tc2_get_next_aux n s l1 l2 l.
Proof.
intros Hg (Hg1, Hg2) (H1, H2); 
  elim l; simpl; auto; clear l.
intros a l Hrec Hi; rewrite Hrec; auto with datatypes.
  rewrite  <- (fun x => tc2_get_number_equiv x s1 s2); auto.
...
Lemma tc2_get_next_movel s l1 l2 :
    match tc2_get_next s l1 l2 with
...
generalize Hrec; case tc2_get_next_aux; auto with datatypes.
Qed.

Lemma tc2_get_next_equiv s s1 s2 l1 l2 :
  valid_state s -> valid_pair (s1, s2) ->
  pstates_equiv (s1,s2) (l1,l2) -> 
    get_next s s1 s2 = tc2_get_next s l1 l2.
Proof.
intros Hg Hg1 H1.
unfold tc2_get_next, get_next.
rewrite (tc2_get_number_equiv _ _ _ _ _ Hg Hg1 H1).
apply tc2_get_next_aux_equiv; auto with datatypes.
Qed.

Lemma tc2_solve_equiv n s s1 s2 l1 l2 :
  valid_state s -> valid_pair (s1, s2) ->
  pstates_equiv (s1,s2) (l1,l2) -> 
    solve n s s1 s2 = tc2_solve n s l1 l2.
Proof.
intros Hs Hg H1; generalize s Hs; elim n; simpl; 
  auto; clear n s Hs.
intros n Hrec s Hs; unfold tc2_step.
rewrite (tc2_get_next_equiv s s1 s2 l1 l2); auto.
generalize (tc2_get_next_movel s l1 l2); case tc2_get_next; auto.
intros m Hm; rewrite Hrec; auto.
apply move_valid with s; auto.
apply Movel_move; auto with datatypes.
Qed.

Lemma tc2_solve_init n s s1 s2 :
  (s1, s2) = fst (iter2 n) ->
  nlreachable n s -> 
  fold_left (fun s a => m2f a s) (tc2_solve n s s1 s2) s = init_state.
Proof.
intros H1 H2.
rewrite <- (tc2_solve_equiv n s (fst (fst (iter2s n))) (snd (fst (iter2s n))) s1 s2).
...
Lemma tc2_solve_length n s s1 s2 :
  (s1, s2) = fst (iter2 n) ->
  valid_state s -> 
  length (tc2_solve n s s1 s2) <= n.
Proof.
intros H1 H.
rewrite <- (tc2_solve_equiv n s (fst (fst (iter2s n))) (snd (fst (iter2s n))) s1 s2); auto.
