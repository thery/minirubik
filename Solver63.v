...
*)
Require Import Int63 List BasicRubik Rubik63.

CoInductive val12 := Mval12: to6 * to6 -> val12.
...
Lemma l11_ok: l11 = OTO6.
Proof.
native_cast_no_check (refl_equal OTO6).
Time Qed.

Lemma s11_correct: (s11, OTO6) = iter 12.
Proof.
...
Proof.
revert s.
assert (F1: forall n ss, (ss, OTO6) = iter n -> 
              to6all ss = true ->
...
CoInductive val11 := Mval11: to6 * to6 * to6 -> val11.
...
Definition solve s := to6_solve 11 s s1 s2.

Theorem solve_eq : forall s, solve s = to6_solve 11 s s1 s2.
Proof.
...
apply trans_equal with
 (fold_left (fun s a => m2f a s) (to6_solve 11 s s1 s2) s).
  apply f_equal3 with (f := @fold_left _ _).
    now apply refl_equal.
    now exact (solve_eq s).
  now apply refl_equal.
apply to6_solve_init.
  now exact s12.
